...
def test_claimed_serialises_a_case_tick(store, clock):
    """The case layer's unit is the tick, not the execute: a tick dispatches containers, so two
    overlapping ticks can start the same worker twice. A duplicated container costs money."""
    from abeyance.claims import claimed

    ran = []
    with claimed(store, "test:case", "c1", owner="host-a", now=clock.now()) as got_a:
        assert got_a is True
        ran.append("a")
        with claimed(store, "test:case", "c1", owner="host-b", now=clock.now()) as got_b:
            assert got_b is False, "a second host must not enter the same tick"
            if got_b:
                ran.append("b")
    assert ran == ["a"]


def test_the_claim_is_released_even_when_the_block_raises(store, clock):
    """A crashed tick must be retryable immediately, not after the lease expires."""
    from abeyance.claims import claimed

    with pytest.raises(RuntimeError):
        with claimed(store, "test:case", "c2", owner="host-a", now=clock.now()) as got:
            assert got
            raise RuntimeError("tick blew up")

    with claimed(store, "test:case", "c2", owner="host-b", now=clock.now()) as got:
        assert got is True


def test_claimed_refuses_a_store_that_cannot_claim(clock):
    ...
    with tempfile.TemporaryDirectory() as d:
        with pytest.raises(TypeError, match="does not support execution claims"):
            with claimed(JSONFileStore(d), "k", "v", owner="a", now=clock.now()):
                pass
